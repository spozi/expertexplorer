...
def show(id):
    ...
    if request.method == "POST":
        ...
        if isMatched:
            conn = mysql.connection
            cursor = conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM candidate WHERE  Avg_sim > 0.45")
            conn.commit()
            total = cursor.fetchall()
            total_count = total[0][0]
            message = ""
            if total_count <= 0:
                message = "All author’s average similarity lower than 0.45"

            page = request.args.get('page', 1, type=int)
            offset = page * limit - limit
            cursor.execute("SELECT * from candidate WHERE Avg_sim > 0.45 LIMIT %s OFFSET %s", (limit, offset))
            conn.commit()
